...
for(const [name,engine] of [['Chromium',chromium],['WebKit',webkit]]){
 ...
 try{
  page=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true,deviceScaleFactor:3});
  await page.addInitScript(()=>{
   ...
   new MutationObserver(records=>{for(const record of records)if(record.target.matches?.('meta[name="viewport"]'))window.viewportChanges.push(record.target.getAttribute('content'))}).observe(document,{subtree:true,attributes:true,attributeFilter:['content']});
  ...
  console.log(name+' input environment',await page.evaluate(()=>({touch:navigator.maxTouchPoints,coarse:matchMedia('(any-pointer:coarse)').matches})));
  ...
  const focus=async(base=1)=>{
   await page.evaluate(scale=>{window.viewportChanges=[];window.testViewport(scale,844/scale)},base);
   await input.focus();
  };
  ...
  await page.waitForFunction(()=>window.viewportChanges.some(v=>/maximum-scale=1(?:,|$)/.test(v)));
  await restored();
  ...
  await page.waitForFunction(()=>window.viewportChanges.some(v=>/maximum-scale=1.25(?:,|$)/.test(v)));
  await restored();
 ...
 }catch(error){console.error(name+' keyboard trace failed:',error,await page.evaluate(()=>({changes:window.viewportChanges,scale:visualViewport.scale,active:document.activeElement?.id})));failures.push(error)}finally{await browser.close()}
}
